...
let me = {
	...
	age: 18,
	greeter: function(){
		'My name is ' +this.firstName+ 'and I live in ' +this.state; 
	}
};
...
function bigOrSmall(arr){
	...
	//1.  loop over arr --which will be a passed in array
	//2. if the number is >100 then .push(big) into the new array
	//3. if the number is <= 100, then .psuh(small) to t
return answers;
}
...
let filtered = myNumbers.filter(function(value){
	return value>100;
})
...
// Last, lets use .forEach to find the index of each item in the array. First, create an empty array called myNumbersIndex. Next, using forEach, push each item's index from the myNumbers array to the myNumbersIndex array

//use for.each
		//to find the index of each item in the array
		//create array myNumbersIndex
		//Use forEach
let myNumbersIndex = [];
...
let enemies = [];
	enemies = people.filter(fucntion(element, index, array){
	 return element.index===false
})
// // console.log(people[0].friend);
//////////////////PROBLEM 20////////////////////
